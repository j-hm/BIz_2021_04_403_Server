...
public class TodoServiceImplV1 implements TodoService {
	...
	private void viewSelect(ResultSet rSet) throws SQLException {
		...
		/*
		 * rSet에는 select table의 결과가 모두 저장되어 있다 실제데이터와 메타데이터가 모두 포함되어 있다 rSet객체의 next()
		 * method를 한번 호출하면 결과 table의 첫번째 레코드를 읽을 수 있도록 준비해준다 이어서 next() method를 계속해서
		 * 호출하면 한줄씩 순서대로 읽을 수 있도록 준비해 준다 만약 더이상 읽을 데이터가 없으면 false return한다 while() 반복문을
		 * 사용하여 select 된 데이터를 순서대로 추출할수 있도록 한다.
		 */
		while (rSet.next()) {
			for (int i = 0; i < columns; i++) {
