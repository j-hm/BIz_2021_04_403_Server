...
public class HomeCommandImplV1 implements TodoCommand{
	 ...
	 * 객체를 클래스의 멤버 영역에 선언한다
	 * 이때 객체의 접근 제한자를 protected로 하면
	 * 이 클래스를 상속받는 곳에서도 공용으로 사용할 수 있다
	 * 이러한 코드가 상당히 이점은 있는데 간혹 메모리 누수가 발생할 수 있다
	 * 이럴 때는 객체를 final로 선언해 주는 것도 한가지 방법이다
	 * 단, 객체를 final로 선언할 경우는 반드시 생성자에서 초기화를 해줘야 한다
	 */
	...
	public void execute(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		RequestCommand.forword(req, res, "home");
