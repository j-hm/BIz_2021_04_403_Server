...
public class FoodServiceImplV1 implements FoodService{
	...
	protected List<MyDTO> select(PreparedStatement pStr) throws SQLException{
		...
		while(rSet.next()) {
			...
			myDTO.setMy_protein(rSet.getInt("단백질"));
			myDTO.setMy_fat(rSet.getInt("지방"));
			myDTO.setMy_carb(rSet.getInt("탄수화물"));
			myDTO.setMy_sugar(rSet.getInt("총당류"));
			myDTO.setMy_totalKcal(rSet.getInt("총칼로리"));
			myList.add(myDTO);
		}
		return myList;
	}
